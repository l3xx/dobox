        ...
        switch($d['type'])
        {
             case dbDynprops::typeRadioGroup:
            {
                ...
                ?><div class="add-ad-<?= $name ?> padTop">
                    <div class="add-ad-capt left"><?= $d['title']; ?></div>
...
//                        }
            ...
            case dbDynprops::typeRadioYesNo:
            {
                ...
                ?><div class="add-ad-<?= $name ?> padTop">
                    <div class="add-ad-capt left"><?= $d['title']?></div>
                    <div class="add-ad-field"><?= '<label><input type="radio" name="'.$name.'" '.$extra.' class="'.$adtxtClass.'" title="Да" value="2" '.($value == 2?'checked="checked"':'').' />Да</label><label><input type="radio" name="'.$name.'" '.$extra.' title="Нет" class="'.$adtxtClass.'" value="1" '.($value == 1?'checked="checked"':'').' />Нет</label>'
                        ?></div>
                  </div> <?php
            }break;             
            ...
            {            
                ...
                ?><div class="add-ad-<?= $name ?> padTop">
                    <div class="add-ad-capt left"><?= $d['title']?></div>
                    <div class="add-ad-field" id="<?= $prefix_id.$d['id']; ?>">
                    <?php      
...
//                        }
            ...
            {   
                ...
                <div class="add-ad-<?= $name ?> padTop">
                    <div class="add-ad-capt left"><?= $d['title']?></div>
                    <div class="add-ad-field">
                        <?= '<select name="'.$name.'" '.$extra.' onchange="addDynpropsParentSelect('.$d['id'].', this.value, \''.$prefix.'\', this);" class="inputText2 '.$adtxtClass.($d['req']?' req':'').'">' ?>
                        <?php
                            foreach($d['multi'] as $dm) {
                ...
                <div class="add-ad-<?= $name ?> padTop">
                    <div class="add-ad-capt left">
                        <?= $d['child_title'].':'.($d['req']?'<span class="req">*</span>':'')?>
                    </div>
                    <div class="add-ad-field">
                        <?= '<select disabled="disabled" name="'.$name.'" '.$extra.' id="'.$prefix_id.$d['id'].'_child" class="inputText2 '.$adtxtClass.($d['req']?' req':'').'"></select>' ?>
                    </div>
                  </div>
                  ...
                  <?php } else { ?>
                      <div class="add-ad-<?= $name ?> padTop">
                        <div class="add-ad-capt left"><?= $d['title']?></div>
                        <div class="add-ad-field">
                            <?= '<select name="'.$name.'" '.$extra.' class="inputText2 '.$adtxtClass.'">' ?>
                            <?php
                            foreach($d['multi'] as $dm) {
                               echo '<option value="'.$dm['value'].'" '.($value == $dm['value'] ? 'selected="selected"' : '').'>'.$dm['name'].'</option>';
                            } ?> 
            ...
            <div class="add-ad-<?= $name ?> padTop">
                <div class="add-ad-capt left"><?= $d['title']?></div>
                <div class="add-ad-field">
                    <?= '<input type="text" name="' . $name . '" ' . $extra . ' value="' . $value . '" class="inputText2 ' . $adtxtClass . ($d['req'] ? ' req' : '') . '" "/>' ?>
                </div>
            </div>
            <div class="clear"></div>
            ...
            <div class="add-ad-<?= $name ?> padTop">
                <div class="add-ad-capt left"><?= $d['title']?></div>
                <div class="add-ad-field">
                    <?= '<textarea type="text" name="' . $name . '" ' . $extra . ' class="inputText2 ' . $adtxtClass . ($d['req'] ? ' req' : '') . '" style="width:652px; height:80px;">' . $value . '</textarea>' ?>
                </div>
            </div>
            <?php
            }break; 
            ...
            case dbDynprops::typeRange:
            {
                ...
                ?><div class="add-ad-<?= $name ?> padTop">
                    <div class="add-ad-capt left"><?= $d['title']?></div>
                    <div class="add-ad-field"><?= '<select name="'.$name.'" '.$extra.' class="inputText2 '.$adtxtClass.($d['req']?' req':'').'">' ?>
                        <?php
                        if(is_string($value)) {
...
function addDynpropsParentSelect(id, val, namePrefix, inputParent)
{
