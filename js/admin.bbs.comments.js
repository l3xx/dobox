var bbsItemsCommentsAdmTreeClass = function(options){ this.initialize(options); } 
bbsItemsCommentsAdmTreeClass.prototype = 
{
    options: {
        path: 'img/',
        img: {          
    ...
    responseNewComment: function(objImg, selfIdComment, bNotFlushNew) {
        ...
        setTimeout(function(){ 
        bff.ajax('index.php?s='+this.options.module+'&ev=items_comments&act=comment-response', 
    ...
    addComment: function(formObj) {    
        var txtarea = $('#form_comment_text');
        if(txtarea.val() == '')  {
            txtarea.focus();
            return;
        }
            
        bff.ajax('index.php?s='+this.options.module+'&ev=items_comments&act=comment-add', $(formObj).serializeArray(),
            function(data) {      
                if(data) {
    ...
    deleteComment: function(link, commentId, callback) {
        ...
        link = $(link);
        bff.ajax('index.php?s='+this.options.module+'&ev=items_comments&act=comment-delete',    
        {'comment_id': commentId, 'rec': this.options.itemID}, function (data) {
            if(data){       
                $('>div.ctext', divContent).html('<span class="desc">Комментарий удален модератором</span>');
                link.remove();
            }
        });
    },
