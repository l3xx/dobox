...
class LiveImage {
	...
	protected $error_messages = array(
		1  => 'Не удалось открыть указанное изображение', //Can`t create image
		2  => 'No font was given',
		3  => 'Не удалось открыть указанное изображение', //No file was given
		4  => 'Не удалось открыть указанное изображение', //Can`t open image from file
		5  => 'Вы загрузили файл неверного формата', //Unknown file format given
		6  => 'Failed image resource given',  //Failed image resource given
        8  => 'Изображение слишком большое по ширине/высоте, загрузите изображение меньшего масштаба'
	);
	/**
	 * Last error text
	 *
	 ...
	 * @return bool
	 */
	public function __construct($file, $iWidthMax=false, $iHeightMax=false) {
		if(!$file || !($size=getimagesize($file))) {
			$this->set_last_error(3);
			return false;
		}
        if( ($iWidthMax!==false && $iWidthMax<=$size[0]) ||
            ($iHeightMax!==false && $iHeightMax<=$size[1]) ) {
                $this->set_last_error(8);
                return false;
            }
		/**
	 ...
	 * @return mixed
	 */
	public function crop($width, $height, $start_width, $start_height, $src_width=false, $src_height=false) {	
		$tmp=($this->truecolor)
			? imagecreatetruecolor($width,$height)
			: imagecreate($width,$height);
		/**
		...
		if($this->format=='gif') { 
            ...
            $src_width = ($src_width!==false ? $src_width : $width);
            $src_height = ($src_height!==false ? $src_height : $height);
			if(!imagecopyresampled($tmp,$this->image,0,0,$start_width,$start_height,$width,$height,$src_width,$src_height)) {
				imagedestroy($tmp);
				return false;
			}
		}
